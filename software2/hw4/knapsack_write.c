...
void make_b(int number, const Itemset *list) {
  //バイナリファイルを作成する。
  int num_list[1] = {number};
  double *d = (double*)malloc(sizeof(double)*number*3);
  for (int i=0; i<number; i++) {
    d[i] = list->value[i];
  }
  for (int i=0; i<number; i++) {
    d[number+i] = list->weight[i];
  }
  char *s = "data.dat";
  FILE *fp;
  fp = fopen(s, "wb"); // bはwindows は必須, linux/mac はなくてもよい
  fwrite(num_list,sizeof(int),1,fp);
  fwrite(d, sizeof(double), number*2, fp);
  fclose(fp);
}
